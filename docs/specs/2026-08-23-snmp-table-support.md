...
- **Column selection.** A "Columns…" control above the grid opens a checkbox list of `TableInfo.column_oids` (index columns marked). Selection persists per table OID in localStorage; default is all columns — under the single-pass walk this is display-only, so there is no network argument for a reduced default. The chosen subset is passed to `snmp_get_table`.
